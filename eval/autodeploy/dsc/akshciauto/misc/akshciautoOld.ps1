configuration AksHciAutoDeploy
{
    ...
    [System.Management.Automation.PSCredential]$nodeLocalCreds = New-Object System.Management.Automation.PSCredential ($adminCreds.Username, $adminCreds.Password)
    ...
    $plainNodePassword = [System.Runtime.InteropServices.Marshal]::PtrToStringBSTR([System.Runtime.InteropServices.Marshal]::SecureStringToBSTR($adminCreds.Password))
    $plainNodeUsername = $adminCreds.Username
    $plainAppID = $aksHciSpCreds.Username
    $plainAppSecret = [System.Runtime.InteropServices.Marshal]::PtrToStringBSTR([System.Runtime.InteropServices.Marshal]::SecureStringToBSTR($aksHciSpCreds.Password))
...
<#
        script "InitializeAksHci" {
            #PsDscRunAsCredential = $nodeLocalCreds
            GetScript            = {
            ...
            SetScript = {
                try {
                    $scriptUsername = "$Using:domainName\administrator"
                    $scriptPassword = ConvertTo-SecureString $Using:plainNodePassword -AsPlainText -Force
                    $scriptCreds = New-Object System.Management.Automation.PSCredential ($scriptUsername, $scriptPassword)
                    $output = Invoke-Command -Credential $scriptCreds -Authentication Credssp -ComputerName localhost -ScriptBlock {
                        Write-Verbose "$(whoami)"
                        Initialize-AksHciNode
                        New-item -Path C:\AksHciAutoDeploy\ -Name "InitializeAksHci.txt" -ItemType File -Force -Verbose
                    }
                    Write-Verbose "Writing output from Initialize-AksHci"
        ...
        script "SetAksHciConfig" {
            ...
            SetScript  = {
                try {
                    ...
                    $scriptPassword = ConvertTo-SecureString $Using:plainNodePassword -AsPlainText -Force
                    $scriptCreds = New-Object System.Management.Automation.PSCredential ($scriptUsername, $scriptPassword)
                    $scriptAksHciNetworking = "DHCP"
                    $scriptTargetAksPath = "V:\AKS-HCI"
                    $output = Invoke-Command -Credential $scriptCreds -Authentication Credssp -ComputerName localhost -ArgumentList $scriptAksHciNetworking, $scriptTargetAksPath -ScriptBlock {
                        $date = (Get-Date).ToString("MMddyy-HHmmss")
                        $clusterRoleName = "akshci-mgmt-cluster-$date"
                        if ($Using:scriptAksHciNetworking -eq "DHCP") {
                            $vnet = New-AksHciNetworkSetting -Name "akshci-main-network" -vSwitchName "InternalNAT" `
                                -vipPoolStart "192.168.0.150" -vipPoolEnd "192.168.0.250"
                        } 
                        else {
                        ...
                        $txtPath = "C:\AksHciAutoDeploy\TestOutput.txt"
                        Remove-Item -Path $txtPath -Confirm:$false -Force -ErrorAction SilentlyContinue -Verbose
                        New-Item "$txtPath" -ItemType file -Force
                        Write-Output "`r`nImageDir = $Using:scriptTargetAksPath\Images, workingDir = $Using:scriptTargetAksPath\WorkingDir and CloudConfig = $Using:scriptTargetAksPath\Config" > $txtPath
                        Set-AksHciConfig -vnet $vnet -imageDir "$Using:scriptTargetAksPath\Images" -workingDir "$Using:scriptTargetAksPath\WorkingDir" `
                        -cloudConfigLocation "$Using:scriptTargetAksPath\Config" -clusterRoleName $clusterRoleName -Verbose
                    }
                    Write-Verbose "Writing output from Set-AksHciConfig"
                    $output = $output | Out-String
                    Write-Verbose "$output"
                }
                catch { throw $_.Exception.Message }
        ...
        script "SetAksHciRegistration" {
            ...
            GetScript  = {
                <#
                #$newCreds = $Using:spCreds
                $newSpSecret = ConvertTo-SecureString "45b963a7-cf38-4b05-b0bf-38eb297d4ea8" -AsPlainText -Force
                $newSPCredentials = New-Object System.Management.Automation.PSCredential ("dce66295-d2a5-4eef-884c-f0541530347f", $newSpSecret)
                Connect-AzAccount -Credential $newSPCredentials -ServicePrincipal -Tenant $Using:tenantId -Subscription $Using:subId -Verbose
                ...
                return @{ 'Result' = $result }
                #>
                $result = Test-Path -Path "C:\AksHciAutoDeploy\RegisterAksHci.txt"
                return @{ 'Result' = $result }
            }
            SetScript  = {
                #$newCreds = $Using:spCreds
                #$newSpSecret = ConvertTo-SecureString "$Using:plainAppSecret" -AsPlainText -Force
                #$newSPCredentials = New-Object System.Management.Automation.PSCredential ($Using:plainAppID, $newSpSecret)
                $newSpSecret = ConvertTo-SecureString "45b963a7-cf38-4b05-b0bf-38eb297d4ea8" -AsPlainText -Force
                $newSPCredentials = New-Object System.Management.Automation.PSCredential ("dce66295-d2a5-4eef-884c-f0541530347f", $newSpSecret)
                Set-AksHciRegistration -SubscriptionId 'ee69155a-1dd3-40aa-ab16-e8480f06c87e' -ResourceGroupName "AksHciDscTest" -TenantId '77ed33f1-9ed5-4479-b300-3bd5a73c7a50' -Credential $newSPCredentials -Verbose
