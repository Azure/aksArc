...
try {
    ...
    az login --identity
    if ($LASTEXITCODE -ne 0) { throw "Failed to login to Azure. Exit code: $LASTEXITCODE" }
    ...
    if ($LASTEXITCODE -ne 0) { throw "Failed to prepare Arc appliance. Exit code: $LASTEXITCODE" }

    Write-Host "Deploying Arc appliance..."
    az arcappliance deploy hci --config-file $configFilePath --outfile $workDirectory\kubeconfig
    if ($LASTEXITCODE -ne 0) { throw "Failed to deploy Arc appliance. Exit code: $LASTEXITCODE" }
    ...
    if ($LASTEXITCODE -ne 0) { throw "Failed to create Arc appliance. Exit code: $LASTEXITCODE" }
